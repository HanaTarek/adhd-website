// api/weekly-report.js  — Vercel Cron Function
...
export default async function handler(req) {
  // Only allow GET requests (what Vercel cron sends)
  if (req.method !== 'GET') {
  ...
  try {
    ...
    const safeRows = rows || []
    const analytics = calculateAnalytics(safeRows)
    const excelBuffer = generateExcel(safeRows, XLSX)
    await sendEmail(analytics, excelBuffer)

    return new Response('Vercel weekly report sent.', { status: 200 })
  } catch (err) {
    console.error('[vercel weekly-report]', err)
    return new Response(`Error: ${err.message}`, { status: 500 })
  }
}

// ── These are the same functions from your weekly-report.js ──────
// Copy calculateAnalytics(), generateExcel(), and sendWeeklyEmail()
// from netlify/functions/weekly-report.js and paste them here.
// Only change: rename sendWeeklyEmail to sendEmail in the call above.
// ================================================================
...
async function sendEmail(a, excelBuffer) {
